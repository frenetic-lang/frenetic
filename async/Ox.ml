...
module Make (Handlers:OXMODULE) = struct
  ...
  let handler (evt:Frenetic.OpenFlow.event) : unit Deferred.t =
    let open Message in
    let open FlowMod in
    ...
    | SwitchUp (sw, feats) ->
      ...
      let res2 = Controller.send sw 1l BarrierRequest in
        (Deferred.both res1 res2 >>| function
          | RpcOk, RpcOk ->
            let sf = Frenetic.OpenFlow.{switch_id = sw; switch_ports = feats} in
            Handlers.switch_connected sw (Frenetic.OpenFlow.To0x01.from_switch_features sf)
          | _ -> ())
    | SwitchDown sw ->
    ...
    | PacketIn (pipe,sw,port,pl,total_len,reason) ->
      let open Frenetic.OpenFlow.To0x01 in
      let pktIn = {
