...
Must process events in reverse order later because appending them.*)
  ...
  let update (nib: Net.Topology.t) (evt:event) : unit =
    ...
    | SwitchDown(switch_id) ->
      ...
      | PortUp (sw_id,pt_id) -> (
        let n1:node = Switch sw_id in
        let node2 = (try let v1 = vertex_of_label nib (Switch sw_id) in
...
module Discovery = struct
  ...
  let start_server (http_port : int) update_policy : unit =
    ...
    let rec collect_stats name =
      (if (!track) then (
        Clock.after (Time.Span.of_sec 4.0)
        >>= fun () ->(let cur_time = Float.to_int (Unix.gettimeofday ()) in
        make_req ("http://localhost:9000/query/"^name) "GET" ()
        >>= fun data ->( let statstr = Yojson.Basic.from_string data in
    ...
    let routes = [
      ...
      ("/switch/([1-9][0-9]*)/port/([1-9][0-9]*)", fun g ->
        ...
        make_req uri "GET" ()
        >>= fun stats -> return (Gui_Server.string_handler stats));
      ("/query/(.*)/pred/(.*)", fun g ->
        let name = Array.get g 1 in
        make_req ("http://localhost:9000/is_query/"^name) "GET" ()
        >>= fun b -> if (b = "true") then
          return (Gui_Server.string_handler "Name already Exists.")
      else(
      ...
      ("/events", fun _ ->
        ...
        return (Gui_Server.string_handler (Yojson.Safe.to_string data)));] in
    (*let converter (p, f) = (Cstruct.t.bytes.of_string p,
          fun bites -> f (Array.map Core.Std.bytes.to_string bites)) in
