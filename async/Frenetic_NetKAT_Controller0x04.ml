...
let of_to_netkat_event fdd (evt : Controller.event) : Frenetic_NetKAT.event list =
  ...
  | `Message (sw_id, hdr, PortStatusMsg ps) ->
      begin match ps.reason, ps.desc.config.port_down with
        | PortAdd, _
        | PortModify, true ->
          let pt_id = (ps.desc.port_no) in
          [PortUp (sw_id, pt_id)]
        | PortDelete, _
        | PortModify, false ->
          let pt_id = (ps.desc.port_no) in
  ...
  | `Message (sw_id,hdr,PacketInMsg pi)
       when pi.pi_port <= Int32.of_int_exn 0xff00 ->
      let port_id = pi.pi_port in
      ...
      List.map pis ~f:(fun (pipe, pkt2) ->
        ...
        in
        PacketIn(pipe, sw_id, port_id, payload, pi.pi_total_len)
      )
  | _ -> []
...
module type CONTROLLER = sig
  ...
  val port_stats : switchId -> portId -> OF.portStats list Deferred.t
...
(* TODO(jcollard): Add support for queries TAG(query)					     
      ...
      switches ~f:(fun sw_id ->
        ...
        Deferred.List.map ~how:`Parallel pats
          ~f:(fun pat ->
            ...
            let req = 
              OF.IndividualRequest
                { sr_of_match = pat0x01; sr_table_id = 0xff; sr_out_port = None } in 
            Controller.send_txn sw_id (OF.Message.StatsRequestMsg req) >>= function
              | `Eof -> return (0L,0L)
              | `Ok l -> begin
                l >>| function
                  | [OF.Message.StatsReplyMsg (IndividualFlowRep stats)] -> 
  ...
  let port_stats (sw_id : switchId) (pid : portId) : OF.portStats list Deferred.t =
    let pt = Int32.(to_int_exn pid) in 
    let req = OF.PortRequest (Some (PhysicalPort pt)) in 
    Controller.send_txn sw_id (OF.Message.StatsRequestMsg req) >>= function
      | `Eof -> assert false
      | `Ok l -> begin
        l >>| function
          | [OF.Message.StatsReplyMsg (PortRep ps)] -> ps
          | _ -> assert false
      end
 ...
 *)
  ...
  let send_pktout ((sw_id, pktout) : switchId * Frenetic_OpenFlow.pktOut) : unit Deferred.t =
    let pktout0x04 = OF.from_packetOut pktout in
    Controller.send sw_id 0l (OF.Message.PacketOutMsg pktout0x04) >>= function
      | `Eof -> return ()
      | `Ok -> return ()
