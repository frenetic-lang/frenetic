...
val send_message : Writer.t -> Frenetic.OpenFlow_Header.xid -> Frenetic.OpenFlow0x04.Message.t -> unit

(* Send group messages to switch to make group table *)
val implement_group_table : Writer.t -> Frenetic.GroupTable0x04.t -> unit

(* Add mask so that the meta value can be changed *)
val mask_meta : int -> int64 Frenetic.OpenFlow0x04.mask

(* Send FlowMod messages to switch to implement policy *)
val implement_flow :
  Writer.t
  -> Netkat.Compiler.t
  -> Netkat.Compiler.flow_layout
  -> Frenetic.OpenFlow.switchId
  -> unit
...
val implement_tolerant_flow :
  Writer.t
  -> Netkat.Compiler.t
  -> Frenetic.Net.Net.Topology.t
  -> Frenetic.OpenFlow.switchId
  -> unit

(* Respond to message from switch *)
val process_message :
  Frenetic.OpenFlow_Header.xid
  -> Frenetic.OpenFlow0x04.Message.t
  -> (Frenetic.OpenFlow_Header.xid -> Frenetic.OpenFlow0x04.Message.t -> unit)
  -> (Frenetic.OpenFlow.switchId -> unit)
  -> unit
...
val main : int -> string -> Netkat.Compiler.flow_layout -> unit -> unit
