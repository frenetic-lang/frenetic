...
let path_to_pnk (topo : Topology.t) (path : path) (v_id, id_v) =
  let (_,_), pol = List.fold_left path
    ~init:((0,0), Drop)
    ~f:(fun ((in_sw, in_pt), pol) edge ->
      let src_sw, src_pt = edge_src edge in
      let dst_sw, dst_pt = edge_dst edge in
      if in_sw = 0 && in_pt = 0 then
        (((VertexMap.find_exn v_id dst_sw), port_to_pnk dst_pt), pol)
      else
        let curr_sw_pol =
          Seq(Test(Switch in_sw),
              Seq(Test(Port in_pt),
                  Seq(Mod(Switch (VertexMap.find_exn v_id src_sw)),
                      Mod(Port (port_to_pnk src_pt))))) in
        if pol = Drop then
          (((VertexMap.find_exn v_id dst_sw), port_to_pnk dst_pt),
          curr_sw_pol)
        else
          (((VertexMap.find_exn v_id dst_sw), port_to_pnk dst_pt), Union(pol,
          curr_sw_pol))) in
  pol
  ...
  ~f:(fun ~key:(src,dst) ~data:path_dist pol_acc ->
    if src = dst then
      ...
      ~f:(fun ~key:path ~data:prob acc ->
        (path_to_pnk topo path (v_id, id_v), prob)::acc) in
      let sd_route_pol =
        Seq (Test(Src src_id),
            Seq (Test(Dst dst_id),
...
let () = begin
  ...
  let net = Seq(Star (Seq (topo_pol, routing_pol)), topo_pol) in
  Dist.print (eval 10 (!!(Switch 101) >> !!(Port 10) >> !!(Src 101) >> !!(Dst 102)
  >> net))
end
