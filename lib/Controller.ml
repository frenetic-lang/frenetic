...
let switch_thread 
  (local_stream : NetKAT_Compiler.SwitchCompiler.t Stream.t)
  (feats : SDN.switchFeatures) : unit Lwt.t =
  let sw_id = feats.SDN.switch_id in
  let config_switch local = 
    let table = NetKAT_Compiler.SwitchCompiler.to_table sw_id local in 
...
let rec start ~port ~pols =
  let local_stream = Stream.map NetKAT_Compiler.SwitchCompiler.compile pols in
