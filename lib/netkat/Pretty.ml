...
module Formatting = struct
  ...
  let format_header_val (fmt : formatter) (hv : header_val) (asgn : string) : unit =
    ...
    | EthSrc(n) -> fprintf fmt "@[ethSrc %s %s@]" asgn (Frenetic_std.Packet.string_of_mac n)
    | EthDst(n) -> fprintf fmt "@[ethDst %s %s@]" asgn (Frenetic_std.Packet.string_of_mac n)
    ...
    | IP4Src(n,32l) -> fprintf fmt "@[ipSrc %s %s@]" asgn (Frenetic_std.Packet.string_of_ip n)
    | IP4Dst(n,32l) -> fprintf fmt "@[ipDst %s %s@]" asgn (Frenetic_std.Packet.string_of_ip n)
    | IP4Src(n,m) -> fprintf fmt "@[ipSrc %s %s/%lu@]" asgn (Frenetic_std.Packet.string_of_ip n) m
    | IP4Dst(n,m) -> fprintf fmt "@[ipDst %s %s/%lu@]" asgn (Frenetic_std.Packet.string_of_ip n) m
...
let string_of_policy = Frenetic_std.Util.make_string_of format_policy

let string_of_pred = Frenetic_std.Util.make_string_of format_pred
