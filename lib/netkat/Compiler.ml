...
let is_valid_pattern options (pat : Frenetic_std.OpenFlow.Pattern.t) : bool =
...
let fill_in_dependencies all_tests (pat : Frenetic_std.OpenFlow.Pattern.t) =
  let open Frenetic_std.OpenFlow.Pattern in
...
let to_pattern hvs =
  List.fold_right hvs ~f:Pattern.to_sdn  ~init:Frenetic_std.OpenFlow.Pattern.match_all

let mk_flows options true_tests all_tests action queries =
  let open Frenetic_std.OpenFlow.Pattern in
  let open Frenetic_std.OpenFlow in
...
let remove_tail_drops fl =
  let rec remove_tail_drop fl =
    ...
    | [] -> fl
    | h :: t ->
      let actions = (fst h).Frenetic_std.OpenFlow.action in
...
let mk_multitable_flow options (pattern : Frenetic_std.OpenFlow.Pattern.t)
...
let subtree_to_table options (subtrees : flow_subtrees) (subtree : (t * flowId))
  (group_tbl : Frenetic_std.GroupTable0x04.t) : multitable_flow list =
...
let subtrees_to_multitable options (subtrees : flow_subtrees) : (multitable_flow list * Frenetic_std.GroupTable0x04.t) =
  let group_table = (Frenetic_std.GroupTable0x04.create ()) in
