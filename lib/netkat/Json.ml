...
let string_of_mac = Frenetic_std.Packet.string_of_mac
let mac_of_string = Frenetic_std.Packet.mac_of_string

let string_of_ip = Frenetic_std.Packet.string_of_ip
let ip_of_string = Frenetic_std.Packet.ip_of_string

let to_json_ip (addr, mask : Frenetic_std.Packet.nwAddr * int32) : json =
...
let from_json_ip (json : json) : Frenetic_std.Packet.nwAddr * int32 =
...
let pattern_to_json (p:Pattern.t) : json =
  ...
  let int_field f = function
    ...
    | Some x -> `Int (f x) in
  `Assoc [
     ("dlSrc", str_field Frenetic_std.Packet.string_of_mac p.dlSrc);
     ("dlDst", str_field Frenetic_std.Packet.string_of_mac p.dlDst);
...
let modify_to_json (m : modify) : json = match m with
  | SetEthSrc m ->
     `List [`String "SetDlSrc"; `String (Frenetic_std.Packet.string_of_mac m)]
  | SetEthDst m ->
     `List [`String "SetDlDst"; `String (Frenetic_std.Packet.string_of_mac m)]
  | SetVlan o ->
  ...
  | SetIP4Src n ->
     `List [`String "SetNwSrc"; `String (Frenetic_std.Packet.string_of_ip n)]
  | SetIP4Dst n ->
     `List [`String "SetNwDst"; `String (Frenetic_std.Packet.string_of_ip n)]
  | SetTCPSrcPort n ->
...
let port_stat_to_json (portStat: Frenetic_std.OpenFlow.portStats) : json =
...
let port_stat_to_json_string (portStat: Frenetic_std.OpenFlow.portStats) : string =
...
let event_to_json (event : event) : json =
  ...
  | PacketIn (pipe, sw_id, pt_id, payload, len, reason) ->
    let buffer = Frenetic_std.OpenFlow.payload_bytes payload |>
      Cstruct.to_string |>
      B64.encode in
    `Assoc [
        ...
        ("payload", `Assoc [
            ...
            ("id", match payload with
              | Frenetic_std.OpenFlow.Buffered (id, _) -> `Int (Int32.to_int_exn id)
              | _  -> `Null)
        ]);
        ("length", `Int len)
    ]
