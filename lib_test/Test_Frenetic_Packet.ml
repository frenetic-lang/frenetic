...
TEST "dlTyp Accessor with TCP payload" = 
  dlTyp sample_tcp_packet = 0x800

TEST "dlTyp Accessor with ARP payload" = 
  dlTyp sample_arp_query_packet = 0x806

TEST "dlTyp Accessor with Unparseable payload" = 
  dlTyp sample_unparseable_packet = 0x6000

TEST "nwSrc Accessor for IP Packet" = 
  nwSrc sample_tcp_packet = 0xc0a80064l

TEST "nwSrc Accessor for ARP Packet" = 
  nwSrc sample_arp_query_packet = ip_of_string "192.168.0.100"

TEST "nwSrc Accessor for Unparseable Packet raises exception" = 
  try nwSrc sample_unparseable_packet = (-1l)
  with | Invalid_argument _ -> true | _ -> false

TEST "nwDst Accessor for IP Packet" = 
  nwDst sample_tcp_packet = 0xac580cfal

TEST "nwDst Accessor for ARP Packet" = 
  nwDst sample_arp_reply_packet = ip_of_string "172.88.12.250"

TEST "nwDst Accessor for Unparseable Packet raises exception" = 
  try nwDst sample_unparseable_packet = (-1l)
  with | Invalid_argument _ -> true | _ -> false

TEST "nwTos Accessor for IP Packet" = 
  nwTos sample_tcp_packet = 141

TEST "nwTos Accessor for Unparseable Packet raises exception" = 
  try nwTos sample_unparseable_packet = (-1)
  with | Invalid_argument _ -> true | _ -> false

TEST "nwProto Accessor for IP Packet" = 
  nwProto sample_tcp_packet = 0x06

TEST "nwProto Accessor for Unparseable IP Packet" = 
  nwProto sample_ip_unparseable_packet = 88

TEST "nwProto Accessor for Unparseable Packet raises exception" = 
  try nwProto sample_unparseable_packet = (-1) 
  with | Invalid_argument _ -> true | _ -> false

TEST "tpSrc Accessor for TCP Packet" = 
  tpSrc sample_tcp_packet = 8080

TEST "tpSrc Accessor for Unparseable IP Packet raises exception" = 
  try tpSrc sample_ip_unparseable_packet = (-1)
  with | Invalid_argument _ -> true | _ -> false

TEST "tpSrc Accessor for Unparseable Packet raises exception" = 
  try tpSrc sample_unparseable_packet = (-1)
  with | Invalid_argument _ -> true | _ -> false

TEST "tpDst Accessor for TCP Packet" = 
  tpDst sample_tcp_packet = 44927

TEST "tpDst Accessor for Unparseable IP Packet raises exception" = 
  try tpDst sample_ip_unparseable_packet = (-1)
  with | Invalid_argument _ -> true | _ -> false

TEST "tpDst Accessor for Unparseable Packet raises exception" = 
  try tpDst sample_unparseable_packet = (-1)
  with | Invalid_argument _ -> true | _ -> false

TEST "arpOperation Accessor for TCP Packet" = 
  arpOperation sample_arp_query_packet = 0x0001

TEST "arpOperation Accessor for Unparseable Packet raises exception" = 
  try arpOperation sample_unparseable_packet = (-1)
  with | Invalid_argument _ -> true | _ -> false

TEST "arpOperation Accessor for IP Packet raises exception" = 
  try arpOperation sample_tcp_packet = (-1)
  with | Invalid_argument _ -> true | _ -> false

(** Mutator *)

TEST "setDlSrc Mutator" = 
  (setDlSrc sample_ethernet_packet 0x102030405060L).dlSrc = 0x102030405060L

TEST "setDlDst Mutator" = 
  (setDlDst sample_ethernet_packet 0x102030405061L).dlDst = 0x102030405061L

TEST "setDlVlan Mutator" = 
  (setDlVlan sample_ethernet_packet None).dlVlan = None

TEST "setDlVlanPcp Mutator" = 
  (setDlVlanPcp sample_ethernet_packet 66).dlVlanPcp = 66

TEST "setNwSrc Mutator sets IP address of IP packet" = 
  nwSrc (setNwSrc sample_tcp_packet (ip_of_string "192.168.0.101")) = (ip_of_string "192.168.0.101")

TEST "setNwSrc Mutator leaves ARP packet alone" = 
  (setNwSrc sample_arp_query_packet (ip_of_string "192.168.0.101")) = sample_arp_query_packet

TEST "setNwDst Mutator sets IP address of IP packet" = 
  nwDst (setNwDst sample_tcp_packet (ip_of_string "172.88.12.251")) = (ip_of_string "172.88.12.251")

TEST "setNwDst Mutator leaves Uparseable packet alone" = 
  (setNwDst sample_unparseable_packet (ip_of_string "172.88.12.251")) = sample_unparseable_packet

TEST "setNwTos Mutator sets Type of Service of IP packet" = 
  nwTos (setNwTos sample_tcp_packet 41) = 41

TEST "setNwTos Mutator leaves ARP packet alone" = 
  (setNwTos sample_arp_reply_packet 41) = sample_arp_reply_packet

TEST "setTpSrc Mutator sets IP address of IP packet" = 
  tpSrc (setTpSrc sample_tcp_packet 8081) = 8081

TEST "setTpSrc Mutator leaves Uparseable packet alone" = 
  (setTpSrc sample_unparseable_packet 8081) = sample_unparseable_packet

TEST "setTpDst Mutator sets IP address of IP packet" = 
  tpDst (setTpDst sample_tcp_packet 17341) = 17341

TEST "setTpDst Mutator leaves Arp packet alone" = 
  (setTpDst sample_arp_query_packet 17341) = sample_arp_query_packet

(** Pretty Printing *)

TEST "string_of_mac returns hh:hh:hh:hh:hh:hh format" = 
  string_of_mac 0x102030405060L = "10:20:30:40:50:60"

TEST "string_of_mac lobs off extra left hand digits" = 
  string_of_mac 0x99102030405060L = "10:20:30:40:50:60"

TEST "mac_of_string converts from hh:hh:hh:hh:hh:hh format" =
  mac_of_string "90:80:70:60:50:40" = 0x908070605040L

TEST "mac_of_string blows up if not in hh:hh:hh:hh:hh:hh format" =
  try mac_of_string "Unparseable" = (-1L)
  with Assert_failure _ -> true | _ -> false  

TEST "string_of_dlAddr returns hh:hh:hh:hh:hh:hh format" = 
  string_of_dlAddr 0x102030405060L = "10:20:30:40:50:60"

TEST "string_of_dlTyp returns readable version of Ethernet Type" =
  string_of_dlTyp 0x800 = "ip"

TEST "string_of_dlTyp returns hex of unknown Ethernet Type" =
  string_of_dlTyp 0x666 = "0x666"

TEST "string_of_dlVlan returns readable version of Vlan" =
  string_of_dlVlan (Some 1356) = "1356"

TEST "string_of_dlVlan returns 'none' if No Vlan is present" =
  string_of_dlVlan None = "none"

TEST "string_of_dlVlanPcp returns readable version of VlanPcp" =
  string_of_dlVlanPcp 800 = "800"

TEST "string_of_ip converts to dotted notation nn.nn.nn.nn" =
  string_of_ip 0xc0a80065l = "192.168.0.101"

TEST "string_of_ip converts 0 to dotted notation 0.0.0.0" =
  string_of_ip 0l = "0.0.0.0"

TEST "ip_of_string converts from dotted notation nn.nn.nn.nn" =
  ip_of_string "192.168.0.101" = 0xc0a80065l

TEST "ip_of_string blows up on inputs not in dotted notation nn.nn.nn.nn" =
  try ip_of_string "192.168.0" = (-1l)
  with Assert_failure _ -> true | _ -> false 

TEST "string_of_nwAddr converts to dotted notation nn.nn.nn.nn" =
  string_of_nwAddr 0xc0a80065l = "192.168.0.101"

TEST "string_of_nwProto returns readable version of IP type" =
  string_of_nwProto (nwProto sample_tcp_packet) = "tcp"

TEST "string_of_nwProto returns integer of IP unparseable type" =
  string_of_nwProto (nwProto sample_ip_unparseable_packet) = "88"  

TEST "string_of_nwTos returns integer of service type" =
  string_of_nwTos (nwTos sample_tcp_packet) = "141" 

TEST "string_of_ipv6 returns IPv6 address of form hhhh:hhhh:hhhh:hhhh:hhhh:hhhh:hhhh:hhhh" = 
  string_of_ipv6 (0x1122334455667788L, 0x9900112233445566L) = "1122:3344:5566:7788:9900:1122:3344:5566"  

TEST "string_of_ipv6 returns IPv6 address in form hhhh:hhhh:hhhh:hhhh:hhhh:hhhh:hhhh:hhhh" = 
  ipv6_of_string "1122:3344:5566:7788:9900:1122:3344:5566" = (0x1122334455667788L, 0x9900112233445566L) 

TEST "string_of_ipv6 fills in 0s between empty ::" = 
...
TEST "ipv6_of_string and string_of_ipv6 are inverses of each other" =
  addr_quickcheck arbitrary_uint128 ipv6_of_string string_of_ipv6

TEST "string_of_tpPort returns integer of TCP src port" =
  string_of_tpPort (tpSrc sample_tcp_packet) = "8080" 

TEST "bytes_of_mac returns packed format" = 
  bytes_of_mac 0x102030405060L = "\x10\x20\x30\x40\x50\x60"

TEST "bytes_of_mac lobs off extra left hand digits" = 
  bytes_of_mac 0x99102030405060L = "\x10\x20\x30\x40\x50\x60"

TEST "mac_of_bytes converts from packed format" =
  mac_of_bytes "\x90\x80\x70\x60\x50\x40" = 0x908070605040L

TEST "mac_of_bytes blows up if not in 6 bytes packed format" =
...
TEST "parse constructs an abstract packet from a packed format buffer" =
...
TEST "len computes length of packet in bytes" = 
  len sample_tcp_packet = 65
    
TEST "marshal constructs a packed format buffer from an abstract packet" =
  marshal sample_tcp_packet = Cstruct.of_string sample_tcp_packet_raw 

TEST "to_string and format_packet gives a representation of the packet" =
...
TEST "Parse and marshall are inverses for TCP packets" =
