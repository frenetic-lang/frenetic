...
TEST "pseudoport_to_json serializes a physical port in json format" = 
  ...
  Yojson.Basic.to_string (pseudoport_to_json phys_port) = "{\"type\":\"physical\",\"port\":6325}"

TEST "pseudoport_to_json serializes a controller port in json format" = 
  let controller_port = Controller 5236 in
  (*printf "%s\n%!" (Yojson.Basic.to_string (pseudoport_to_json controller_port));*)
  Yojson.Basic.to_string (pseudoport_to_json controller_port) = "{\"type\":\"controller\",\"bytes\":5236}"

TEST "pseudoport_from_json parses a physical port from json format" = 
  let phys_port_json_str = Yojson.Basic.from_string "{\"type\":\"physical\",\"port\":6325}" in
  (pseudoport_from_json phys_port_json_str) = Physical 6325l

TEST "pseudoport_to_json serializes a controller port in json format" = 
  let controller_port_json_str = Yojson.Basic.from_string "{\"type\":\"controller\",\"bytes\":5236}" in
...
TEST "pkt_out returns switch and abstract Packet Out message from Json format" =
  ...
  let sample_packet_out = pkt_out_from_json (Yojson.Basic.from_string sample_packet_out_str) in
  sample_packet_out = ( 
    843509872345L, 
...
TEST "pkt_out doesn't support modify action" =
  let sample_packet_out_modify_json = Yojson.Basic.from_string "{ \"actions\": [ {\"type\": \"modify\"} ] }" in
  try 
...
TEST "flowTable_to_json serializes a flow table in Json format" = 
  ...
  Yojson.Basic.to_string (flowTable_to_json ft) = (In_channel.read_all "lib_test/data/flow_table_nightmare_pattern.json") 

TEST "flowTable_to_json doesn't handle multiple actions" = 
  let ft = Test_Frenetic_OpenFlow.sample_flow_table in
  try 
    Yojson.Basic.to_string (flowTable_to_json ft) = ""
  with Failure _ -> true | _ -> false 
