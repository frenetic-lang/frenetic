...
TEST "create creates a fresh group table" = 
  let gtab = create () in
  (to_string gtab) = ""
...
TEST "add_group adds a group to a group table" = 
  let gtab = create () in
...
TEST "clear_group removes all groups from a group table" = 
  let gtab = create () in
  let _ = add_group gtab Select rr_bucket_list in
...
TEST "commit emits OpenFlow 1.3 messages to recreate the group table on the switch" = 
  let gtab = create () in
  let _ = add_group gtab Select rr_bucket_list in
  let () = clear_groups gtab in
...
TEST "add_fastfail_group forms fast fail group from a list of ports, most important first, creating appropriate action buckets" = 
  let gtab = create () in
