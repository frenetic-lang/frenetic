...
(*
  let%test "S.mk_branch is a primitive to create an if-then-else node for the diagram" = 
    let () = init in
  ...
  let%test "S.mk_branch does a quick optimization if the true and false branches are equal" = 
    let () = init in
    let ten_idx = mk_leaf 10 in
    unget (mk_branch (Var1,30) ten_idx ten_idx) = Leaf 10

  let%test "S.mk_leaf is a primitive to create a leaf node for the diagram" = 
    init;
    unget (mk_leaf 60) = Leaf 60 
*)
  let%test "S.mk_leaf doesn't make the same leaf node twice" = 
    let () = init in
    let one_idx = mk_leaf 60 in
    let two_idx = mk_leaf 60 in
    one_idx = two_idx
