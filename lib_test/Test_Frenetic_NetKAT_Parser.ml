...
let%test "policy_of_string returns a simple policy from a policy string in examples/drop.kat" =
  let nk_str = In_channel.read_all "examples/drop.kat" in
  policy_of_string nk_str = Filter False

let%test "policy_of_string returns a policy from a policy string in examples/example1.kat" =
  let nk_str = In_channel.read_all "examples/example1.kat" in
  policy_of_string nk_str = union3 
    (Seq (filter_port(1l), ( Union (output_port(2l), output_port(3l)) ))) 
    (Seq (filter_port(2l), output_port(1l)))
    (Seq (filter_port(3l), output_port(1l)))

let%test "policy_of_string returns a policy from a policy string in examples/fall-through-optimization.kat" =
  let nk_str = In_channel.read_all "examples/fall-through-optimization.kat" in
  policy_of_string nk_str = if_then_else 
