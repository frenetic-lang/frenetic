...
let parse_pretty str = 
  string_of_policy (Frenetic_NetKAT_Parser.policy_from_string str)

let test_parse p_str = 
  try 
    let _ = Frenetic_NetKAT_Parser.policy_from_string p_str in
    true
  with _ ->
...
let test_pretty p_str =
  let p_ast1 = Frenetic_NetKAT_Parser.policy_from_string p_str in
  let p_str' = string_of_policy p_ast1 in
  let p_ast2 = Frenetic_NetKAT_Parser.policy_from_string p_str' in
  p_ast1 = p_ast2
...
let prop_parse_pol_idempotent (p : policy) : bool =
  try 
    ...
    let p' = Frenetic_NetKAT_Parser.policy_from_string s in 
