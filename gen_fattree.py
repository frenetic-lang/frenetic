		...
		for i in range(num_layers,0,-1):
			...
			for j in range(len(temp2)):
				for k in range(num_fan_out):
					...
					if p == 1:
						g.add_edge(temp2[j],temp1[num_fan_out * j + k],attr_dict={'sport':1,'dport':k+1,'capacity':'1Gbps','cost':'1'})
					else:
						for l in range(p):
							g.add_edge(temp2[j],temp1[num_fan_out * j + k],attr_dict={'sport':p+l+1,'dport': num_fan_out * k + l+1,'capacity':'1Gbps','cost':'1'})
