...
let to_flow_mod prio pat act0 =
  ...
  (concat_map (translate_action ofMatch.matchInPort) act0); mfCookie =
  Word64.zero; mfIdleTimeOut = Permanent; mfHardTimeOut = Permanent;
  mfNotifyWhenRemoved = false; mfApplyToPacket = None; mfOutPort = None;
  ...
  let config_commands pol0 swId =
    sequence
      (map (fun fm -> Monad.send swId Word32.zero (FlowModMsg fm))
        (delete_all_flows::(flow_mods_of_classifier (compile_opt pol0 swId))))
