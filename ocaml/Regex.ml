...
let get_hop topo s1 s2 : int

let flatten_reg pol = match pol with
  | Hop sw -> [Hop sw]
  | Star -> [Star]
  (* | Option reg1 reg2 -> [Option reg1 reg2] *)
  | Sequence reg1 reg2 -> flatten_reg reg1 ++ flatten_reg reg2

let collapse_star pol = match pol with
...
*)
let compile1 pred reg topo = match reg with
  | Hop s1 :: Hop s2 :: reg -> Par (Pol (And pred (Switch s1)) [To (get_hop topo s1 s2)]) (compile1 pred (Hop s2 :: reg) topo)
