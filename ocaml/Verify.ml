...
let () = 
  ...
  let pol0 = Seq (Filter (And(And (Switch s1, DlSrc s1), DlDst s2)), Act (To 2)) in
  let pol1 = Seq (Filter (Switch s3), Act ToAll) in
  let pol2 = Seq (Filter (Switch s2), Act ToAll) in
